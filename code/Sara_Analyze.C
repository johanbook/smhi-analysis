...
void Sara_Analyze(){
		...
		//open file1 
		 ifstream file1("./year_hotday_coldday.dat");
		...
		//open file2
		 ifstream file2("years_days_temps.dat");
		 Int_t count2=0;
		...
		//TH1D* hCold = new TH1D("hCold", "Distribution of the coldest days; Day ; Counts", 365, 1, 366);
		////fill the histograms
			//for(Int_t i = 0; i < 285; i++) {
			//hHot->Fill(Array_hotdays[i]);
			//hCold->Fill(Array_colddays[i]);
			//}
		  ...
		  ////hHot>SetLineWidth(1);
		  ////hHot->Draw();
		  //// create canvas for hCold
		  //TCanvas* cold = new TCanvas("cold", "Cold canvas", 900, 600);
		  //hCold->SetMinimum(0);
		  //hCold->SetFillColor(kBlue-7);
		  //hCold->SetLineColor(kBlue-7);
		...
		//TF1* funcCold= new TF1("Gaussian", Gaussian, 1, 366, 3);
		//funcCold->SetParameters(5, 200, 60); //Starting values for fitting
		//funcCold->SetLineColor(kBlack);
		//funcCold->SetLineWidth(1);
		//funcCold->SetLineStyle(2);
		...
		//leg->AddEntry(hCold, "", "F"); //Use custom title
		//hHot->Draw();
		//hCold->Draw("SAME"); //Draw on top of the existing plot
		//leg->Draw(); //Legends are automatically drawn with "SAME"
		
		  //// Save the canvas as a picture
		  ////hot->SaveAs("Distr_Hottestdays.jpg");
		  //cold->SaveAs("Distr_Coldestdays.jpg");
	...
	////2nd part:How hottest and coldest days changed over the years
	   ...
	   //TCanvas *graph1DHot = new TCanvas("graph1DHot","1D Graph year-day_Hot",200,10,700,500);
		...
		//grHot->GetYaxis()->SetLimits(150,250);
